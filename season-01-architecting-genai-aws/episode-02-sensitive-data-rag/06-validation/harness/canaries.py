"""Canaries and the eligibility oracle, generated from the fixtures (VALIDATION_PLAN principle 2).
...
oracle against the hand-written table in 03-architecture/SYNTHETIC_DATA_MODEL.md §3, so a wrong rule cannot hide by
...
def oracle(record_overrides=None):
...
def by_canary(table=None):
    return {e["canary"]: key for key, e in (table or oracle()).items()}


def ineligible_canaries(persona, table=None):
    return sorted(e["canary"] for e in (table or oracle()).values() if persona not in e["eligible"])


def expected_inventory(table=None):
    table = table or oracle()
