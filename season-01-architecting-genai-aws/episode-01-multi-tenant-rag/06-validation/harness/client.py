...
class Response:
    def __init__(self, status, body, headers, elapsed_ms=None):
        self.status = status
        self.body = body
        self.headers = {k.lower(): v for k, v in headers.items()}
        self.event_id = self.headers.get("x-tla-event-id")
        self.request_id = self.headers.get("apigw-requestid")
        self.elapsed_ms = elapsed_ms  # client-observed, including network (NFR-001 evidence; not a benchmark)
    ...
    def summary(self):
        return {"status": self.status, "code": self.code(), "event_id": self.event_id, "request_id": self.request_id,
                "client_elapsed_ms": self.elapsed_ms}
    ...
    def call(self, method, path, token=None, body=None, headers=None, authorization=None, query=""):
        ...
        for attempt in range(4):
            ...
            if status == 429 and attempt < 3:
                ...
                started = time.monotonic()  # latency of the attempt that produced the recorded answer
                continue
            break
        ...
        return Response(status, parsed, reply_headers, elapsed_ms)
