...
class SourceRules(unittest.TestCase):
    ...
    @unittest.skipUnless(os.path.isdir(SENSITIVITY), "failure-experiment modules are not part of this package")
    def test_every_variant_is_marked(self):
        for name in ("status_ignored.py", "order_ignored.py", "no_reconciliation.py"):
            self.assertIn("SENSITIVITY VARIANT — TEST ONLY", open(os.path.join(SENSITIVITY, name)).read())

    def test_logs_carry_identifiers_and_counts_never_content(self):
        """Every log line is a JSON dump of a literal dict of identifiers and counts — never document text."""
        forbidden = re.compile(r"\b(text|body|question|answer|prompt|content|markdown|section_body)\b")
        for path, text in sources():
            for match in re.finditer(r"print\((.*)\)", text):
                printed = match.group(1)
                self.assertTrue(printed.startswith("json.dumps("), f"{path}: {printed[:90]}")
                self.assertNotRegex(printed, forbidden, path)

    def test_only_reconciliation_advances_a_watermark(self):
        """ADR-003: a notification says what we received; only a completed pass may say what is accounted for."""
        callers = [path for path, text in sources()
                   if "advance_watermark(" in text and path != os.path.join("adapters", "stores.py")]
        self.assertEqual(callers, [os.path.join("change", "reconciler.py")])

    def test_the_request_path_never_writes_derived_state(self):
        for relative in ("query/handler.py", "query/retrieval_gateway.py", "query/policy_decision.py"):
            text = open(os.path.join(APP, relative), encoding="utf-8").read()
            for writer in ("put_pending", "clear_pending", "put_reflected", "put_generation", "advance_watermark"):
                self.assertNotIn(writer, text, f"{relative} calls {writer}")
...
class TemplateRules(unittest.TestCase):
    ...
    def test_only_the_change_path_can_write_the_index(self):
        """ADR-004: one writer of derived retrieval state, so ordering and completeness are proved in one place."""
        self.assertEqual(self.holders("bedrock:IngestKnowledgeBaseDocuments"), ["ChangeRole"])
        self.assertEqual(self.holders("bedrock:DeleteKnowledgeBaseDocuments"), ["ChangeRole"])

    def test_the_notifier_may_write_pending_entries_and_nothing_else(self):
        """§7 in the template: the role that hears about changes cannot advance a watermark."""
        writes = [s for s in statements(self.resources["NotifierRole"])
                  if s["Effect"] == "Allow" and "dynamodb:PutItem" in actions(s)]
        self.assertTrue(writes)
        for statement in writes:
            self.assertIn("PENDING#*", repr(statement.get("Condition")))
    ...
    def test_section_bucket_policies_restrict_reads_and_writes(self):
        for tier in ("Shared", "Restricted"):
            ...
            self.assertIn("ChangeRole", policy)
