...
class Observer:
    ...
    def change_reports(self):
        """What the change path recorded, oldest first. Episode 02's ingestion reports are now change applications.

        Ordered by TIMESTAMP: the request id is a uuid, so sorting by it puts "the latest report" in an arbitrary
        place — which is exactly how a caller asking for the most recent application of a document gets an old one.
        """
        return sorted((i for i in self.audit_items() if i.get("record_type") == "CHANGE_APPLICATION"),
                      key=lambda i: (i.get("timestamp") or "", i.get("request_id") or ""))
    ...
    def log_groups(self):
        ...
        return [f"/aws/lambda/{prefix}-{name}" for name in
                ("query", "notifier", "applier", "reconciler", "rebuild")] + [f"/tla/{prefix}/api-access"]
