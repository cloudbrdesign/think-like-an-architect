...
EPISODE = "03"          # the tag every deployment carries; a stale value here would report clean whatever remained
...
def verify(session, account, variants):
    ...
    # The records stream → notifier mapping outlives a failed stack deletion, and it is the change signal itself.
    found["event_source_mappings"] = [
        m["UUID"] for page in session.client("lambda").get_paginator("list_event_source_mappings").paginate()
        for m in page["EventSourceMappings"] if starts((m.get("FunctionArn") or "").split(":function:")[-1])]
    ...
    for page in session.client("resourcegroupstaggingapi").get_paginator("get_resources").paginate(TagFilters=[
            {"Key": "Course", "Values": ["ThinkLikeAnArchitect"]}, {"Key": "Episode", "Values": [EPISODE]}]):
