...
class Context:
    def __init__(self, target, run):
        ...
        self.eligibility = canaries.eligibility_table()
    ...
    def ineligible(self, persona):
        return set(canaries.ineligible_canaries(persona, self.eligibility))

    def retrieved_canaries(self, observation):
        return {self.eligibility[k]["canary"] for k in observation.retrieved_keys if k in self.eligibility}
